...
class Condition_Test < Test::Unit::TestCase
    ...
    def test_initiaize
        ...
        val2 = ValueNode.new(2)

        greater = LogicExpr.new(val1)
        lesser = LogicExpr.new(val2)
...
class LogicStmt_Test < Test::Unit::TestCase
    ...
    def test_and
        ...
        #TEST :logicExpr "&&" :logicExpr
        toBeOrNotToBe3 = LogicStmt.new(value1, "&&", value2)
...
class ValueCompTest < Test::Unit::TestCase
    ...
    def test_greater_less
        val1 = ValueNode.new("5")
        val2 = ValueNode.new("2")

        greater = LogicExpr.new(val1)
        lesser = LogicExpr.new(val2)

        valNode = ValueComp.new(greater, ">", lesser)
        assert_equal(true, valNode.eval())

        valNode2 = ValueComp.new(lesser, "<", greater)
...
class LogicExpr_Test < Test::Unit::TestCase
    ...
    def test_initiaize
        ...
        assert_equal(10, conNode.eval(scope))

        tempVal1 = VariableNode.new("test")
        conNode = LogicExpr.new(tempVal1)
        assert_equal(nil, conNode.eval(scope))
        ...
        assert_equal(19, conNode.eval(scope))
        
        varNode = VariableNode.new("temp")
        scope.addVariable(varNode, tempVal)
